...
const MemoryDatabase = require('./mem-db');
...
class ModelLayer {
  constructor ({databaseURL}) {
    ...
    if (this.databaseURL) {
        ...
        this.mem_db = new MemoryDatabase(1000);
    }
  }
  ...
  async createPortal ({hostPeerId}) {
    ...
    if (this.db) {
        ...
        this.mem_db.insert(id, hostPeerId)
    }
  ...
  async findPortal (id) {
    ...
    try {
        if (this.db) {
            ...
            console.log({hostPeerId: this.mem_db.get(id)});
            let res_host_peer_id = {hostPeerId: this.mem_db.get(id)};
